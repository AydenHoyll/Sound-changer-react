...
function ApplyRule(inword, r) {
  ...
  if (t.substr(0, 2) == "-*") {
    if (showinterm || intonly) addToInterm(inword);
    return inword;
  }
    ...
    if (Match(inword, i, thisrule[0], thisrule[2])) {
      ...
      if (thisrule.length > 3) {
        ...
        if (slix != -1) {
          ...
          if (
            gix - precount >= 0 &&
            Match(inword, gix - precount, thisrule[0], thisrule[3])
          ) {
...
function DoWords() {
  ...
  for (w = 0; w < nlex; w++) {
    ...
    if (inword.length > 0) {
      ...
      if (localshowdiff && w < oldolex.length && outs != oldolex[w]) {
        outs = outs.bold();
      }
  ...
  if (dle != null) {
    dle.setAttribute(
      "href",
      "data:text/plain;charset=utf-8," + encodeURIComponent(olex)
    );
...
function allowOops() {
  ...
  if (oopsButton != null) oopsID.style.display = "block";
}
...
function openFile(event) {
  ...
  reader.onload = function () {
    ...
    if (rulr > ruln) {
      var newS = "";
      for (var w = 0; w < rulr.length; w++) newS += rulr[w] + "\n";
      theText = newS;
    }
